  ...
  Spr.aurelith = function (frame) {
    return cached('au:' + frame, function () {
      return Px.make(56, 60, function (g) {
        var cx = 28, flap = frame ? 5 : 0;

        /* Six wings, three per side, mirrored at the end. Each pivots where
           it meets the carapace, so the root holds and the tip swings.

           It used to shift each whole wing up by a flat `flap * i` - the pair
           furthest out moved most, the root moved exactly as far as the tip,
           and the topmost pair, which is the longest and the one you actually
           read, multiplied by i = 0 and never moved at all. Three rigid bars
           sliding is not a wingbeat. */
        for (var i = 0; i < 3; i++) {
          var yy = 14 + i * 7;
          var len = 22 - i * 3;
          for (var x = 0; x < len; x++) {
            var span = x / (len - 1);
            /* rounded together, or the sweep and the lift disagree by a pixel
               every few columns and the wing reads as a dashed line */
            var drop = Math.round(x * 0.42 - span * span * flap) + i * 2;
