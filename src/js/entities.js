...
(function (V) {
  ...
  function Enemy(defId, x, y, world, opts) {
    ...
    /* WHAT IT IS TRYING TO FACE, as opposed to where its body has got to.
       Keeping these apart is the whole of the fix below: an enemy used to set
       facing = bearing-to-player every frame, so stepping behind one was
       answered in the same frame by it already looking at you. */
    this.aim = this.facing;
    this.reactTimer = 0;
    /* Mass turns slower and notices later. Both derive from radius so the rule
       holds for anything added later without a table to keep in sync; `turn`
       and `react` in the def override it where a creature is meant to break
       the rule - a shade is light on its feet, an ironclad is a door. */
    this.turnRate = def.turn || U.clamp(96 / (def.radius + 6), 2.2, 8);
    this.reactTime = def.react != null ? def.react
                                       : U.clamp(def.radius * 0.014 + 0.12, 0.12, 0.5);
    this.state = 'idle';
    this.stateTime = 0;
  ...
  Enemy.prototype.update = function (dt) {
    ...
    switch (this.state) {
      ...
      case 'chase': {
        ...
        /* IT HAS TO BE LOOKING AT YOU TO START. Range alone used to be enough,
           which was fine when facing snapped to the bearing in the same frame -
           with a real turn rate it meant an enemy could commit to a swing while
           still coming about, and miss a player standing perfectly still. It
           keeps closing and turning until it is lined up; the gate is inside
           the swing's own half-arc so a shot it commits to is one it can land. */
        if (d < this.def.attackRange &&
            Math.abs(U.angleDelta(this.facing, toP)) < this.swingArc() * 0.4) {
          this.state = 'windup'; this.stateTime = 0; this.attackedThisSwing = false;
        }
      ...
      case 'windup': {
        this.moving = false;
        /* IT COMMITS. Tracking all the way through the wind-up meant a dodge
           behind it was answered by the swing simply following you round, so
           the telegraph was decoration - there was no position the wind-up
           could be punished from. It aims over the first third and is locked
           after that, which is what makes getting behind one worth doing. */
        if (this.stateTime < this.def.attackWindup * 0.35) this.face(toP, dt);
  ...
  /* Turn toward a bearing, with a beat before it accepts a big change and a
     finite rate once it does.

     Two things were wrong. The chase state assigned facing = toP outright, so
     there was no turn at all - dodging round someone was answered instantly,
     which is what makes a body feel like a turret. And the wind-up used
     U.lerp(facing, ..., 0.18) with no dt, so how fast an enemy tracked you
     depended on the frame rate: twice as fast at 120fps as at 60.

     `react` is the part that reads as thought. A small correction is free -
     it is already following you - but a bearing that jumps by more than a
     quarter turn means something happened, and it costs a beat before the
     body commits to the new direction. That beat is the dodge paying off. */
  var REACT_ARC = Math.PI * 0.45;      /* how far you must get round it to count */

  Enemy.prototype.face = function (toP, dt) {
    var drift = Math.abs(U.angleDelta(this.aim, toP));
    if (this.reactTimer > 0) {
      this.reactTimer -= dt;
      if (this.reactTimer <= 0) this.aim = toP;   /* now it commits */
    } else if (drift > REACT_ARC) {
      this.reactTimer = this.reactTime;          /* noticed; has not moved yet */
    } else {
      this.aim = toP;                            /* small corrections are free */
    }

    var d = U.angleDelta(this.facing, this.aim);
    var max = this.turnRate * dt;
    this.facing += Math.abs(d) <= max ? d : (d < 0 ? -max : max);
  };
  ...
  /* One definition, because the wind-up gate below and the swing itself have
     to agree: an enemy that commits to a swing it cannot land is not a
     creature that commits, it is one that misses. */
  Enemy.prototype.swingArc = function () { return this.def.boss ? 1.6 : 1.1; };
