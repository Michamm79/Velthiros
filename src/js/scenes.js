...
(function (V) {
  ...
  function StartScene(game) {
    ...
    this.t = 0;
    this.idle = 0;
    /* Once it has surfaced it does not go back in the box. It used to live only
       on the scene instance, so walking into Help and back rebuilt StartScene,
       cleared the flag and started another five-minute wait - the secret found
       you and then un-found you. */
    this.promptShown = !!game.save.comboSeen && !game.save.scytheUnlocked;
  ...
  StartScene.prototype.update = function (dt) {
    ...
    if (!this.promptShown && !this.game.save.scytheUnlocked && this.idle >= this.game.idleUnlockSeconds) {
      ...
      this.comboProgress = 0;
      this.game.save.comboSeen = true;
      this.game.persist();
      Audio.play('warp');
    }

    /* Arrows are live on the title screen whether or not the panel is up, so
       someone who KNOWS the sequence can just enter it. Gating input on the
       prompt made the secret worthless to know: you still had to sit through
       five minutes of nothing before the game would listen. Polled here rather
       than in drawCombo so it works with the panel closed - keyPressed is
       edge-triggered and cleared at the end of the frame, so one read only. */
    if (!this.game.save.scytheUnlocked && !this.showInstallHelp) {
  ...
  StartScene.prototype.pressDir = function (dir) {
    if (this.game.save.scytheUnlocked) return;
  ...
  StartScene.prototype.render = function (ctx, cw, ch) {
    ...
    if (this.promptShown) this.drawCombo(ctx, cw, ch, s);
      ...
      if (this.game.save.comboSeen) {
        /* Found once is found. The mark sits under the menu from then on and
           reopens the panel, so the only thing the five minutes ever bought
           you was knowing it was there. */
        /* Below the menu, not over it. At ch*0.33 it sat squarely on top of
           Continue - menuBottom is already measured here for Garatu, so use
           the same edge rather than another guessed fraction. */
        if (UI.button(ctx, 'cmb_open', uiX - 22 * s, menuBottom + 10 * s, 44 * s, 30 * s, '\u25C7',
  ...
  StartScene.prototype.drawCombo = function (ctx, cw, ch, s) {
    ...
    /* 322 rather than 300, and centred on the new height: the 'later' button
       sat at 288 and is 26 tall, so it hung 14px through the panel's floor. */
    ...
    UI.text(ctx, 'enter it', cw / 2, y + 268 * s, { size: 12, align: 'center', colour: 'rgba(255,255,255,0.45)' });

    /* A way out that is not "wait another five minutes". Closing it only hides
       it for this visit - save.comboSeen keeps it found, so the mark below the
       menu brings it straight back. */
